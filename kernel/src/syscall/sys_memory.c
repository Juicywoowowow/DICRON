...
long sys_mmap(long addr, long length, long prot, long flags, long fd,
  ...
  uint64_t va_start = proc->mmap_next;
  if (va_start == 0)
    va_start = 0x0000200000000000ULL; /* init if not set */

  uint64_t alloc_size = num_pages * PAGE_SIZE;
  if (va_start + alloc_size >= USER_SPACE_END ||
      va_start + alloc_size < va_start)
    return -ENOMEM;

  proc->mmap_next = va_start + alloc_size;
...
long sys_munmap(long addr, long length, long a2, long a3, long a4, long a5) {
  (void)a2;
  (void)a3;
  (void)a4;
  (void)a5;
  ...
  if (va_end < va_start || va_end > USER_SPACE_END)
    return -EINVAL;
